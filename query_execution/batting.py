...
def batting_data(source_conn, destination_conn):
    logger.info("Batting table data validation START")

    # to fetch distinct match id
    query = 'select DISTINCT(Match_Id) from BCCI.clear.PFT_BCCI_BATTING where Match_Id != \' \''
    ...
    for math_id in match_ids:
        ...
        source_df = pd.read_sql(
            f'{read_sql_file("./query_resources/source/batting.sql")} where Match_Id = \'{math_id}\'', source_conn)
        ...
        destination_df = pd.read_sql(
            f'{read_sql_file("./query_resources/destination/batting.sql")} and EX_Match_Id = \'{math_id}\'',
            destination_conn)
        ...
        try:
            ...
            source_sort_df.to_csv(f'./report/batting/{math_id.replace(" ", "_")}/source.csv', index=False)
            destination_sort_df.to_csv(f'./report/batting/{math_id.replace(" ", "_")}/destination.csv', index=False)
            #logger.error(f"COUNT MISMATCH original error {error}")

    report.Report.append({"batting": {"src_total_count": src_total_count, "dst_total_count": dst_total_count,
                                        "diff_count": src_total_count - dst_total_count}})
    logger.info(report.Report)
    logger.info("Batting table data validation END")
