...
def fow_data(source_conn, destination_conn):
    logger.info("fow table data validation START")
    # to fetch distinct match id
    query = 'select DISTINCT(Match_Id) from BCCI.clear.pft_bcci_fow where Match_Id != \' \''

    # Extracting all match ids in listsssss
    ...
    for math_id in match_ids:
        ...
        source_df = pd.read_sql(
            f'{read_sql_file("./query_resources/source/fow.sql")} and Match_Id = \'{math_id}\'', source_conn)
        ...
        destination_df = pd.read_sql(
            f'{read_sql_file("./query_resources/destination/fow.sql")} and EX_Match_Id = \'{math_id}\'',
            destination_conn)
        ...
        try:
            ...
            if result.size != 0:
                ...
                make_directory(f'./report/fow')
                result.to_csv(f'./report/fow/{math_id}.csv', index=False)

        except Exception as error:

            # adding csv file for both source and destination table
            make_directory(f'./report/fow/{math_id.replace(" ", "_")}')
            source_sort_df.to_csv(f'./report/fow/{math_id.replace(" ", "_")}/source.csv', index=False)
            destination_sort_df.to_csv(f'./report/fow/{math_id.replace(" ", "_")}/destination.csv', index=False)
            logger.error(f"COUNT MISMATCH original error {error}")

    report.Report.append({"fow": {"src_total_count": src_total_count, "dst_total_count": dst_total_count, "diff_count": src_total_count - dst_total_count }})
    logger.info(report.Report)
    logger.info("fow table data validation END")
